...
counter = 0
data_dir = "/home/elia/UWB-Results/example"  # Garage
...
def insertMissingPoints(z,k,diff):
    new_time = np.linspace(z.time_stamps[k], z.time_stamps[k+1], diff + 1)

    # finding the average distance between 2 points in x
    px = np.linspace(z.x[k+1], z.x[k], diff + 1)
    for i in range(len(px)): px[i] = np.round(px[i],2)
    # finding the average distance between 2 points in y
    py = np.linspace(z.y[k+1], z.y[k], diff + 1)
    for i in range(len(py)): py[i] = np.round(py[i],2)
    np.round(py,2)

    data = {"time_stamps":new_time[:diff],"x":px[:diff],"y":py[:diff]}
    rows = pd.DataFrame(data,columns=["time_stamps","x","y"])
...
for i in range(1, len(sub_folders), 1):
    ...
    for i in csv_files:
        ...
        GPS = pd.read_csv(
            "/home/elia/UWB-Results/RTK_GPS_Terrace/GPS.csv")
        a = UWB
        ...
        for k in range(1, len(z.index)-1,1):
